...
export default function ContactSection() {
    ...
    const onSubmit = async (data: ContactFormValues) => {
        try {
            ...
            //  Modificación clave:  Construir el objeto con los datos y convertirlo a JSON
            const formData = {
            ...
            const response = await fetch("/api/contact", { // Use /api/contact
                method: "POST",
                headers: {
            ...
            if (response.ok) {
                ...
                reset();
                router.push("/gracias");  // Use router.push for navigation
            } else {
                ...
                try {
                    ...
                    // If parsing the error response as JSON fails, use a generic message
                }
            ...
            toast({
                variant: "destructive",
                description:
                    error.message || "Error al enviar el mensaje. Intente nuevamente más tarde.",
            });
        } finally {
            setIsLoading(false);
        }
